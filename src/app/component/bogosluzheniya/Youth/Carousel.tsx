...
import { Box, Button, Paper, Modal, IconButton, Typography } from '@mui/material'
...
export function CustomImageGallery() {
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      const generalDoc = await client.getSingle<Content.YouthDocument>('youth')
      const gallery = generalDoc.data.images || []

      const imageFields = gallery
        .map((item: Content.YouthDocumentDataImagesItem) => item.image)
        .filter((img: ImageField): img is ImageField => !!img && !!img.url)
  ...
  return (
    <>
      ...
      >
        ...
        {/* Вертикальный скролл с миниатюрами */}
        <Box 
          position="relative" 
          width={264} 
          height={448} 
